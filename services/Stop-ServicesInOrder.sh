...
# Bash alternative to Stop-ServicesInOrder.ps1
# Stops services in order with timeout and polling
...
stop_service_with_timeout() {
    ...
    if $STATUS_CMD "$service_name" | grep -q "inactive\|dead\|stopped"; then
        echo "SUCCESS: Service '$service_name' is already stopped."
        return 0
    fi
    
    # 2. Attempt to stop
    echo "Status is running. Attempting to stop service..."
    ...
    while [ $(($(date +%s) - start_time)) -lt $TOTAL_TIMEOUT_SECONDS ]; do
        ...
        if echo "$current_status" | grep -q "inactive\|dead\|stopped"; then
            echo "SUCCESS: Service '$service_name' stopped and is stopped."
            return 0
        fi
    ...
    # 4. Timeout failure
    echo "FAILURE: Service '$service_name' failed to stop after $TOTAL_TIMEOUT_MINUTES minutes."
    return 1
}
...
for service_name in "${SERVICE_NAMES[@]}"; do
    if ! stop_service_with_timeout "$service_name"; then
...
echo ""
echo "All specified services stopped successfully."
exit 0
